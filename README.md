...
- Copies a PR link to the clipboard with one keystroke, ready to paste and share — mark multiple rows to copy them all at once. On macOS this matches AzDO's own "Copy link" format (e.g. pasting into Teams renders "Pull Request 38549: Title" as a clickable link); other platforms copy the plain title text.
...
| `Space` | Mark/unmark the highlighted PR for a multi-select |
| `Alt+U` | Copy marked PRs' links to the clipboard (or just the highlighted one if none are marked) |
| `Alt+R` | Refresh now |
| `Alt+Q` or `Esc` | Quit |
